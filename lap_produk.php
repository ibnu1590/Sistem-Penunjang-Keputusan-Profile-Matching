        ...
        public function Header() {
            ...
            $this->writeHTMLCell(
              ...
              $html = '
                ...
                <table>
                <tr>
                <td align="center" style="font-size: 15px;">Laporan Data Produk</td>
                </tr>
                </table>
                <table>
                <tr>
...
<thead>
    ...
    <tbody>';
        $no=1; 
        foreach ($db->select('produk.nama_produk,kriteria.nama_kriteria,profile_produk.nilai_produk','profile_produk,produk,kriteria')->where('profile_produk.id_kriteria=kriteria.id_kriteria AND profile_produk.id_produk=produk.id_produk')->get() as $data):
            $tmp = explode('_',$data['nama_kriteria']);
            $dataKriteria = ucwords(implode(' ',$tmp));
            $htmlTable .='<tr>
            <td>'.$data['nama_produk'].'</td>
            <td>'.$dataKriteria.'</td>
            <td>'.$data['nilai_produk'].'</td>
        </tr>';
        $no++; endforeach;
        $htmlTable .= '</tbody>
    </table>';
